public class Main {
   ...
   public static void main(String []args) {
      ...
      // ARRAY is a DATA STRUCTURE (collection) that
      ...
      int[] luckyNumbers; // just creates a named REFERENCE, not values yet
      double[] mileTimes;
      boolean[] trueOrFalse;
      // Arrays can also store OBJECTS (reference types)
      String[] spiritAnimals;

      // TWO OPTIONS to CREATE the array
      ...
      //arrayName[index] --> gets an item at that position
      ...
      faveAnimals[1] = "dog";
      // NOTE that the other 6 indicies hold NULL
      ...
      // So wr can use the same index for multiple lists
      ...
      // WATCH OUT for the bounds with the STOP conditon!
      ...
      // Example: Mondify existing array values
      for (int i = 0; i < bestClass.length; i++) {
      ...
      // LIMITATIONS:
      // can't mondify values when using for-each
      // becayse we don't keep track of INDEX
      // For-each loops is good for "visiting every item" in order. Otherwise, use a standard loop!!!
